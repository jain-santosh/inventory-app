...
const IconTruck = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <path strokeLinecap="round" strokeLinejoin="round" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10h10v1a2 2 0 002 2h2m-2-4h.808c.227 0 .393.185.393.411 0 .227-.166.411-.393.411H14m-5 4v-4m-8-12h17.172a2 2 0 011.414.586l3.364 3.364A2 2 0 0121 11.414V15a1 1 0 01-1 1h-1M5 16h10V8H5v8z" />
    </svg>
);
...
const IconRefresh = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 0012 4.088C8.94 4.887 6.4 7.397 5.228 10.662m15.004 3.784L19.5 19h-5.084M4 13.784l.5-4.888m15.004 3.784L19.5 19h-5.084" />
    </svg>
);
...
const DEFAULT_METAL_RATES = {
    'Gold': 65.00,       // Example rate: $65.00 per gram
    'Platinum': 35.00,   // Example rate: $35.00 per gram
    'Silver': 0.80,      // Example rate: $0.80 per gram
    'White Gold': 60.00, // Example rate: $60.00 per gram
    // All rates are assumed per gram
};
const DEFAULT_TAX_RATE = 5; // Example GST rate
...
/**
 * Calculates the base material value of an item based on its net metal weight and current market rates.
 */
function calculateBaseValue(item, metalRates) {
    ...
    // Assuming stone values and making charges are added as "cost" to base value if available
...
export default function App() {
    ...
    const [statusFilter, setStatusFilter] = useState('In Stock'); // Default to In Stock
    ...
    useEffect(() => {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (user) {
                setUserId(user.uid);
                await checkAndAssignRole(user.uid);
                setIsAuthReady(true);
            } else {
                try {
    ...
    const checkAndAssignRole = async (uid) => {
        if (!db) return;
    ...
    return (
        <div className="min-h-screen bg-gray-100 font-sans text-gray-800 pb-20">
                    ...
                    <div className="flex justify-between items-center h-16">
                        ...
                        {activeTab === 'inventory' && (
                            <button
                                ...
                                className="hidden sm:flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                            >
                                <IconPlus />
                                <span>Add Item</span>
                            </button>
            ...
            <main className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl mt-4 sm:mt-8">
                ...
                {!isAuthReady || (isLoading && inventory.length === 0 && activeTab !== 'settings' && activeTab !== 'users' && activeTab !== 'suppliers') ? (
...
/**
 * NEW MODULE: Point of Sale (POS) View
 */
function POSView({ items, metalRates, taxRate, onSaleComplete }) {
    ...
    const handleCompleteSale = () => {
        if (!selectedItem) {
            alert('Please select an item for sale.');
            return;
        }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <FormSelect
                        label="Inventory Item (SKU - Name - Category)"
                        name="selectedItem"
                        value={selectedItemId}
                        onChange={(e) => setSelectedItemId(e.target.value)}
                        // FIX: Change options format to be an array of strings (ids)
                        options={[''].concat(availableItems.map(item => item.id))} 
                    >
                    ...
                    {availableItems.length === 0 && (
                        <p className="mt-4 text-red-500 font-medium">No items available for sale in stock.</p>
                    )}
                </div>
                ...
                {selectedItem && (
                    <div className="bg-white p-6 rounded-xl shadow-md border border-gray-300">
                        ...
                        <div className="flex gap-4 sm:gap-6">
                            <img 
                                className="h-24 w-24 rounded-lg object-cover bg-gray-100 flex-shrink-0" 
                                src={selectedItem.imageUrl || 'https://placehold.co/100x100/e2e8f0/94a3b8?text=Item'} 
                                alt={selectedItem.name} 
                                onError={(e) => { e.target.src = 'https://placehold.co/100x100/e2e8f0/94a3b8?text=Item'; }}
                            />
                            <div className="flex-1">
                                ...
                                <div className="text-xs text-gray-500 mt-1">
                                    {selectedItem.metals?.map(m => `${m.netWeight}g ${m.purity} ${m.type}`).join(', ')}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {/* Right Side: Calculation & Checkout */}
                ...
                <div className="bg-blue-50 p-6 rounded-xl shadow-lg border border-blue-200 sticky top-20 space-y-4">
                    ...
                    <div className="flex flex-col gap-3 pt-4">
                        <button
                            ...
                            disabled={!selectedItem || finalPrice <= 0}
                            className="w-full px-4 py-3 bg-green-600 text-white rounded-lg text-lg font-semibold shadow-md hover:bg-green-700 transition duration-150 disabled:bg-gray-400 disabled:cursor-not-allowed"
                        >
                            Complete Sale
                        </button>
                        <button
                            onClick={handleReset}
                            type="button"
                            className="w-full px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition duration-150"
                        >
                            Reset
                        </button>
                    </div>
...
/**
 * NEW: Suppliers and Purchase Order Management View
 */
function SuppliersView({ suppliersCollectionPath, purchaseOrdersCollectionPath, metalConfig, categories, openItemForm }) {
    ...
    // Constants for PO Material dropdown
    ...
    const availableMaterials = useMemo(() => {
        ...
        return STONE_TYPES;
    }, [newPOData.type, availableMetals, availableCategories, isEditingPO, editingPO]); // Corrected dependency: availableMetals
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(collection(db, suppliersCollectionPath), (snapshot) => {
            ...
            // FIX: Set initial supplier ID only if a supplier list is fetched AND the ID is currently empty.
            if (list.length > 0 && !newPOData.supplierId) {
                setNewPOData(prev => ({ ...prev, supplierId: list[0].id }));
            }
        });
        return () => unsubscribe();
    ...
    const handleAddSupplier = async (e) => {
        ...
        try {
            await addDoc(collection(db, suppliersCollectionPath), { name: newSupplierName.trim(), contact: '', addedAt: new Date().toISOString() });
            setNewSupplierName('');
        } catch (e) { console.error("Error adding supplier:", e); }
    };

    const handleDeleteSupplier = async (id) => {
        if (window.confirm("Are you sure you want to delete this supplier? This will not delete related POs.")) {
            try {
                await deleteDoc(doc(db, suppliersCollectionPath, id));
    ...
    const handlePOChange = (e) => {
        ...
        // If type changes, reset materialName
        if (name === 'type') {
            update.materialName = '';
        }
        
        // If editing, update local editing state
        if (isEditingPO && editingPO) {
            setEditingPO(prev => ({ ...prev, ...update }));
        } else {
            // If adding new, update new PO state
            setNewPOData(prev => ({ ...prev, ...update }));
        }
    };
    ...
    const handleSavePO = async (e) => {
        ...
        if (!poToSave.supplierId || !poToSave.quantity || !poToSave.type || !poToSave.materialName) {
        ...
        const dataToSave = {
            ...
            quantity: parseFloat(poToSave.quantity),
        };
        ...
        try {
            if (isEditingPO) {
                ...
                // Reset form state for new PO entry
                setNewPOData(prev => ({ ...prev, quantity: '', materialName: '' }));
            }
    ...
    const handleUpdatePOStatus = async (po, newStatus) => {
        if (po.status === 'Completed' && newStatus === 'Completed') return; // Prevent re-triggering completion

        try {
            // 1. Update PO Status
            await setDoc(doc(db, purchaseOrdersCollectionPath, po.id), { status: newStatus }, { merge: true });

            // 2. If status is Completed, Reflect in Inventory and open edit form
            if (newStatus === 'Completed') {
                const newStockItem = await reflectPOInInventory(po);
                if (newStockItem) {
                    openItemForm(newStockItem); 
                }
            }
        } catch (e) { 
    ...
    const reflectPOInInventory = async (po) => {
        ...
        if (po.type === 'Metal') {
            baseItem.metals = [{
                type: po.materialName,
                purity: metalConfig[po.materialName]?.[0] || '',
                netWeight: po.quantity.toFixed(2),
                grossWeight: po.quantity.toFixed(2),
            }];
            baseItem.category = 'Raw Metal';
        } else if (po.type === 'Stone') {
            baseItem.stones = [{
                type: po.materialName,
                carat: po.quantity.toFixed(2),
                cut: 'Round', color: 'G', clarity: 'SI1' // Default stone details
            }];
        ...
        try {
            ...
            return { id: docRef.id, ...baseItem }; // Return the new item to open the form
        } catch (e) {
            console.error("Error reflecting PO in inventory:", e);
            alert("PO completed, but failed to create basic inventory item.");
            return null;
        }
    };
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b bg-gray-50">
                    <h3 className="text-lg font-medium text-gray-900">Supplier Database</h3>
                </div>
                    ...
                    <div className="md:col-span-1">
                        <form onSubmit={handleAddSupplier} className="space-y-4 p-4 border rounded-lg bg-gray-50">
                            ...
                            <input
                                ...
                                placeholder="Vendor Name (e.g., ABC Bullion)"
                        ...
                        <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                            {suppliers.map(supplier => (
                                <div key={supplier.id} className="flex justify-between items-center p-3 bg-white border rounded-lg shadow-sm">
                                    ...
                                    <button onClick={() => handleDeleteSupplier(supplier.id)} className="text-red-500 hover:text-red-700 p-1 rounded-full hover:bg-red-50">
                                        <IconTrash />
                                    </button>
                                </div>
                            ))}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b bg-gray-50">
                    <h3 className="text-lg font-medium text-gray-900">Purchase Order (PO) Tracking</h3>
                </div>
                
                <div className="p-6 space-y-6">
                    {/* Add/Edit PO Form */}
                    <form onSubmit={handleSavePO} className="grid grid-cols-1 sm:grid-cols-6 gap-4 p-4 border-b pb-6">
                        <div className="sm:col-span-6 text-lg font-semibold text-gray-700 mb-2">
                            {isEditingPO ? `Edit PO ${editingPO.orderNumber}` : 'Create New PO'}
                        </div>
                        ...
                        <div className="sm:col-span-1 flex items-end gap-2">
                            <button type="submit" className="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                                {isEditingPO ? 'Update PO' : 'Add PO'}
                            </button>
                            {isEditingPO && (
                                <button type="button" onClick={closeEditPO} className="w-full px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400">
                                    Cancel
                                </button>
                            )}
                        </div>
                    ...
                    {purchaseOrders.length === 0 && (
                        <p className="text-center text-gray-500 py-6">No purchase orders added yet.</p>
                    )}
                </div>
            </div>
        </div>
...
/**
 * NEW: User Management View (Admin Only) - Now Responsive
 */
function UserManagementView({ usersCollectionPath }) {
    ...
    const handleChangeRole = async (userId, newRole) => {
        if (window.confirm(`Change role to ${newRole}?`)) {
            try {
                await setDoc(doc(db, usersCollectionPath, userId), { role: newRole }, { merge: true });
            } catch(e) {
    ...
    const RoleBadge = ({ role }) => (
        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
            role === ROLES.ADMIN ? 'bg-purple-100 text-purple-800' :
            role === ROLES.MANAGER ? 'bg-blue-100 text-blue-800' :
                ...
                {users.map(user => (
                    <div key={user.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-500">Joined: {user.joinedAt ? new Date(user.joinedAt).toLocaleDateString() : '-'}</span>
...
function SettingsView({ categories, metalConfig, metalCategoryMap, metalRates, taxRate, publicDataPath }) {
    ...
    // Local state for Global Rates
    const [localMetalRates, setLocalMetalRates] = useState(metalRates);
    const [localTaxRate, setLocalTaxRate] = useState(taxRate);

    useEffect(() => {
        setLocalMetalRates(metalRates);
        setLocalTaxRate(taxRate);
    }, [metalRates, taxRate]);

    // --- Global Rates Logic ---
    const handleMetalRateChange = (metalType, value) => {
        setLocalMetalRates(prev => ({ 
            ...prev, 
            [metalType]: parseFloat(value) || 0 
        }));
    };
    ...
    const handleAddCategory = async (e) => {
        ...
        try {
            await setDoc(doc(db, `${publicDataPath}/categories`, newCategory.trim()), { name: newCategory.trim() });
            setNewCategory("");
        } catch(e) { console.error(e); alert("Error adding category"); }
    };

    const handleDeleteCategory = async (catName) => {
        if(window.confirm(`Delete category "${catName}"? Note: This will not remove the category from existing inventory items.`)) {
             try {
                await deleteDoc(doc(db, `${publicDataPath}/categories`, catName));
                
                const updatedMap = { ...metalCategoryMap };
                Object.keys(updatedMap).forEach(metal => {
                    updatedMap[metal] = updatedMap[metal].filter(c => c !== catName);
                });
                await setDoc(doc(db, `${publicDataPath}/settings`, 'metalCategoryMap'), updatedMap);

            } catch(e) { console.error(e); alert("Error deleting category"); }
        }
    };
    ...
    const handleAddMetal = async (e) => {
        ...
        try {
            ...
            await setDoc(doc(db, `${publicDataPath}/settings`, 'metalCategoryMap'), updatedMap);
            await setDoc(doc(db, `${publicDataPath}/settings`, 'globalRates'), { metalRates: updatedRates, taxRate: localTaxRate });
    ...
    const handleDeleteMetal = async (metalName) => {
        if(window.confirm(`Delete Metal "${metalName}" and all its purity options and mappings?`)) {
             const updatedConfig = { ...metalConfig };
             delete updatedConfig[metalName];
             const updatedMap = { ...metalCategoryMap };
             delete updatedMap[metalName];
             const updatedRates = { ...localMetalRates };
             delete updatedRates[metalName];
    ...
    // --- Purity Logic ---
    const handleAddPurity = async (e) => {
        e.preventDefault();
        if (!newPurity.trim() || !selectedMetalForPurity) return;
        
        const currentPurities = metalConfig[selectedMetalForPurity] || [];
        if (currentPurities.includes(newPurity.trim())) return;

        const updatedConfig = { 
            ...metalConfig, 
            [selectedMetalForPurity]: [...currentPurities, newPurity.trim()] 
        };
    ...
    const handleDeletePurity = async (metal, purity) => {
         if(window.confirm(`Remove purity "${purity}" from ${metal}?`)) {
             const currentPurities = metalConfig[metal];
             const updatedConfig = {
                 ...metalConfig,
                 [metal]: currentPurities.filter(p => p !== purity)
             };
              try {
                await setDoc(doc(db, `${publicDataPath}/settings`, 'metals'), updatedConfig);
    ...
    const handleToggleMapping = async (metalName, categoryName) => {
        const currentMappedCategories = metalCategoryMap[metalName] || [];
        let updatedCategories;

        if (currentMappedCategories.includes(categoryName)) {
            updatedCategories = currentMappedCategories.filter(c => c !== categoryName);
        } else {
            updatedCategories = [...currentMappedCategories, categoryName].sort();
        }

        const updatedMap = {
            ...metalCategoryMap,
            [metalName]: updatedCategories
        };
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Global Rates Card (NEW) */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-red-50">
                    <h3 className="text-lg font-bold text-red-800">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 inline mr-2 align-text-bottom" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5l-2 4A1 1 0 007 14h6a1 1 0 00.867-1.5l-2-4A1 1 0 0010 7z" clipRule="evenodd" /></svg>
                        Critical: Live Pricing & Tax Rates
                    </h3>
                    <p className="text-sm text-red-600">These rates are used for all POS calculations.</p>
                </div>
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        {Object.keys(localMetalRates).map(metal => (
                            <div key={metal} className="flex justify-between items-center gap-4">
                                ...
                                <div className="flex flex-1 items-center border border-gray-300 rounded-lg shadow-sm">
                                    ...
                                    <input
                                        type="number"
                                        value={localMetalRates[metal] || ''}
                                        onChange={(e) => handleMetalRateChange(metal, e.target.value)}
                                        step="0.01"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-4">
                            <label className="text-sm font-medium w-24">Tax Rate:</label>
                            <div className="flex flex-1 items-center border border-gray-300 rounded-lg shadow-sm">
                                <input
                                    type="number"
                                    value={localTaxRate}
                                    ...
                                    min="0"
                                    max="100"
                                    className="w-full px-2 py-2 focus:outline-none focus:ring-0 rounded-l-lg"
                                />
                ...
                <div className="p-6 border-t bg-gray-50 flex justify-end">
                    <button onClick={handleSaveGlobalRates} className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-semibold">
                        Save Global Rates & Tax
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    ...
                    <p className="text-sm text-gray-500">Manage product types (e.g., Ring, Bangle)</p>
                </div>
                <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto p-1">
                        {categories.map(cat => (
                            <span key={cat} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800">
                                {cat}
                                <button onClick={() => handleDeleteCategory(cat)} className="ml-2 text-gray-400 hover:text-red-600">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 className="text-lg font-medium text-gray-900">Metals & Purity</h3>
                    <p className="text-sm text-gray-500">Manage metal types and their purity options</p>
                </div>
                <div className="p-6 space-y-6">
                    
                    {/* Add New Metal Type */}
                    <div className="border-b pb-6">
                        <h4 className="text-sm font-semibold text-gray-700 mb-3">Add New Metal Type</h4>
                        <form onSubmit={handleAddMetal} className="flex gap-2">
                             <input 
                                type="text" 
                                value={newMetal}
                                ...
                                className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                            <button type="submit" className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Add Metal</button>
                        </form>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mb-4">
                            <select 
                                value={selectedMetalForPurity}
                                onChange={(e) => setSelectedMetalForPurity(e.target.value)}
                                className="px-3 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
                                {Object.keys(metalConfig).map(m => <option key={m} value={m}>{m}</option>)}
                            </select>

                            <form onSubmit={handleAddPurity} className="flex gap-2 flex-1">
                                <input 
                                    type="text" 
                                    value={newPurity}
                                    onChange={(e) => setNewPurity(e.target.value)}
                                    placeholder={`Add purity for ${selectedMetalForPurity}`}
                                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                />
                                <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Add</button>
                            </form>
                        ...
                        {/* List of Purities for Selected Metal */}
                        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                             <div className="flex justify-between items-center mb-3">
                                <span className="font-medium text-gray-700">{selectedMetalForPurity} Purities:</span>
                                <button onClick={() => handleDeleteMetal(selectedMetalForPurity)} className="text-xs text-red-600 hover:underline">
                                    Delete {selectedMetalForPurity} Metal
                                </button>
                             </div>
                             <div className="flex flex-wrap gap-2">
                                {metalConfig[selectedMetalForPurity]?.map(purity => (
                                    <span key={purity} className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-white border border-gray-300 text-gray-700 shadow-sm">
                                        {purity}
                                        <button onClick={() => handleDeletePurity(selectedMetalForPurity, purity)} className="ml-1.5 text-gray-400 hover:text-red-600">
                                ...
                                {(!metalConfig[selectedMetalForPurity] || metalConfig[selectedMetalForPurity].length === 0) && (
                                    <span className="text-sm text-gray-400 italic">No purities defined. Add one above.</span>
                                )}
                             </div>
                        </div>
                    </div>
            ...
            {/* Metal to Category Mapping Card (NEW) */}
            <div className="md:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                 <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 className="text-lg font-medium text-gray-900">Metal Compatibility Mapping</h3>
                    <p className="text-sm text-gray-500">Select which categories can use a specific metal. This filters the list in the Item Form.</p>
                </div>
                <div className="p-6 space-y-4">
                    <select 
                        value={selectedMetalForMapping}
                        onChange={(e) => setSelectedMetalForMapping(e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4"
                    >
                        {Object.keys(metalConfig).map(m => <option key={`map-${m}`} value={m}>{m}</option>)}
                    </select>
                        ...
                        {categories.map(cat => {
                            ...
                            return (
                                <button
                                    key={`map-cat-${cat}`}
                                    type="button"
                                    onClick={() => handleToggleMapping(selectedMetalForMapping, cat)}
                    ...
                    {categories.length === 0 && (
                        <p className="text-center text-gray-400 py-4">Please add categories in the top section first.</p>
                    )}
                </div>
            </div>
...
/**
 * New Module: Stock Report & Details View (Aggregated by Category & Metal)
 */
function StockReport({ items }) {
    const stats = useMemo(() => {
        const result = {
            totalItems: 0,
            metals: {},
            groupedStock: {} // Stores grouped data by Category + Metal
        };
        ...
        items.forEach(item => {
            ...
            if (item.status !== 'Sold') {
                ...
                // Determine Primary Metal (first metal or 'Unknown')
                let primaryMetal = 'Unknown Metal';
                if (item.metals && item.metals.length > 0) {
                    primaryMetal = item.metals[0].type;
                }

                // Create Composite Key: "Category_PrimaryMetal"
                const catName = item.category || 'Uncategorized';
                const groupKey = `${catName}_${primaryMetal}`;

                if (!result.groupedStock[groupKey]) {
                    result.groupedStock[groupKey] = {
                        name: catName,
                        primaryMetal: primaryMetal,
                        count: 0,
                        gross: 0,
                        net: 0,
                        metalTypes: new Set() 
                ...
                if (item.metals) {
                    item.metals.forEach(m => {
                        ...
                        const n = parseFloat(m.netWeight || 0);

                        // Global Metal Stats (for top cards)
                        if (!result.metals[m.type]) {
                            result.metals[m.type] = { gross: 0, net: 0 };
                        }
                        result.metals[m.type].gross += g;
                        result.metals[m.type].net += n;

                        // Group Specific Stats
                        result.groupedStock[groupKey].gross += g;
                        result.groupedStock[groupKey].net += n;
    ...
    // Convert categories object to array for rendering and sort
    const stockRows = Object.values(stats.groupedStock).sort((a, b) => {
        // Sort by category name, then by metal
        ...
        <div className="space-y-6 sm:space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                    <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Total In-Stock</div>
                    <div className="mt-1 text-2xl font-bold text-gray-900">{stats.totalItems} <span className="text-sm font-normal text-gray-400">Items</span></div>
                </div>

                {Object.entries(stats.metals).map(([type, data]) => (
                    <div key={type} className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
                        <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">{type} Stock</div>
                        <div className="mt-1 flex justify-between items-end">
                            <div>
                                <span className="text-xl font-bold text-gray-900">{data.net.toFixed(2)}</span>
                                <span className="ml-1 text-xs text-gray-500">g Net</span>
                            </div>
                            <div className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded">
                                {data.gross.toFixed(2)}g Grs
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {/* Mobile View: Aggregated Cards */}
            <div className="block sm:hidden space-y-4">
                <div className="flex justify-between items-center px-1">
                    <h3 className="text-lg font-medium text-gray-900">Category Summary</h3>
                    <span className="text-xs text-gray-500 bg-gray-200 px-2 py-1 rounded">
                        {stockRows.length} Groups
                    </span>
                </div>
                    ...
                    <div key={idx} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <div className="text-sm text-gray-500">{group.primaryMetal}</div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 border-t pt-4">
                            <div>
                                <div className="text-xs text-gray-500 uppercase">Gross Weight</div>
                                <div className="text-gray-700 font-mono font-medium">{group.gross.toFixed(2)}g</div>
                            </div>
                            <div className="text-right">
                                <div className="text-xs text-blue-600 uppercase font-bold">Net Weight</div>
                                <div className="text-blue-700 font-mono text-xl font-bold">{group.net.toFixed(2)}g</div>
                            </div>
                        </div>
                    </div>
                ))}
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Metal Type</th>
                                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Quantity</th>
                                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-100">Total Gross (g)</th>
                                <th className="px-6 py-3 text-right text-xs font-bold text-gray-700 uppercase tracking-wider bg-blue-50">Total Net (g)</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200 text-sm">
                            {stockRows.map((group, idx) => (
                                <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                    ...
                                    <td className="px-6 py-4 text-right text-gray-600 bg-gray-50/50 font-mono">
                                        {group.gross.toFixed(2)}
                                    </td>
                                    <td className="px-6 py-4 text-right font-bold text-blue-700 bg-blue-50/30 font-mono text-base">
                                        {group.net.toFixed(2)}
                                    </td>
                                </tr>
                            ))}
                        ...
                        {stockRows.length > 0 && (
                            <tfoot className="bg-gray-100 font-semibold text-gray-900">
                                <tr>
                                    ...
                                    <td className="px-6 py-3 text-center">{stats.totalItems}</td>
                                    <td className="px-6 py-3 text-right">
                                        {stockRows.reduce((sum, group) => sum + group.gross, 0).toFixed(2)}
                                    </td>
                                    <td className="px-6 py-3 text-right text-blue-800 bg-blue-100">
                                        {stockRows.reduce((sum, group) => sum + group.net, 0).toFixed(2)}
                                    </td>
                                </tr>
                            </tfoot>
                        )}
...
/**
 * Inventory List Table Component (Responsive)
 */
function InventoryList({ items, onEdit, onDelete, userRole }) {
    if (items.length === 0) {
        return (
                ...
                {items.map((item) => (
                    <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex gap-4">
                        ...
                        <div className="flex-shrink-0">
                             <img 
                                className="h-20 w-20 rounded-lg object-cover bg-gray-100 border border-gray-100" 
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 font-mono mt-0.5">{item.sku} • {item.category}</p>
                                
                                <div className="mt-2 text-sm text-gray-700">
                                    {item.metals && item.metals.length > 0 && (
                                        <span className="block">
                                            {item.metals[0].grossWeight}g <span className="text-xs text-gray-500">{item.metals[0].purity}</span>
                                        </span>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex justify-end gap-3 mt-2">
                                <button 
                                    onClick={() => onEdit(item)} 
                                    className="text-blue-600 hover:text-blue-800 text-xs font-medium flex items-center gap-1 px-2 py-1 rounded bg-blue-50"
                                >
                                    Edit
                                </button>
                                {userRole !== ROLES.STAFF && (
                                    <button 
                                        ...
                                        className="text-red-600 hover:text-red-800 text-xs font-medium flex items-center gap-1 px-2 py-1 rounded bg-red-50"
                                    >
                                        Delete
                                    </button>
                                )}
 ...
 */
function ItemForm({ item, categories, metalConfig, metalCategoryMap, onSave, onClose, userId, appId, userRole }) { 
    // Determine the initially selected metal type
    const initialMetalType = (item?.metals?.[0]?.type || Object.keys(metalConfig)[0] || 'Gold');

    // Use useMemo to keep defaultState reference stable unless its dependencies change
    const defaultState = useMemo(() => ({
        ...
        imageUrl: '',
        metals: [{ type: initialMetalType, purity: '', netWeight: 0, grossWeight: 0, makingCharges: 0, stoneValue: 0 }],
        stones: []
    }), [initialMetalType]);

    // Set initial purity based on initial metal type
    if (defaultState.metals[0].type && metalConfig[defaultState.metals[0].type]) {
         defaultState.metals[0].purity = metalConfig[defaultState.metals[0].type][0] || '';
    }

    const [formData, setFormData] = useState(defaultState);

    // Calculate available categories based on the first selected metal
    ...
    // 1. Init Form Data when Item changes (Only runs when item ID changes)
    useEffect(() => {
        if (item) {
            // Apply defaults to handle missing fields from older items or PO conversion
            const updatedMetals = item.metals?.map(m => ({
                ...m,
                makingCharges: parseFloat(m.makingCharges || 0),
                stoneValue: parseFloat(m.stoneValue || 0)
            })) || defaultState.metals;
    ...
    // 2. Update Category when Available Categories change (due to metal change)
    useEffect(() => {
        const currentCategory = formData.category;
        
        if (currentCategory && !availableCategories.includes(currentCategory)) {
             const newCategory = availableCategories.length > 0 ? availableCategories[0] : '';
    ...
    const handleMetalChange = (index, e) => {
        ...
        let newValue = value;
        if (name === 'makingCharges' || name === 'stoneValue' || name === 'netWeight' || name === 'grossWeight') {
    ...
    const addMetal = () => {
        const firstMetal = Object.keys(metalConfig)[0] || 'Gold';
        setFormData(prev => ({
            ...prev,
            metals: [...prev.metals, { type: firstMetal, purity: metalConfig[firstMetal]?.[0] || '', netWeight: 0, grossWeight: 0, makingCharges: 0, stoneValue: 0 }]
        }));
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center sm:items-start sm:pt-16 p-4 sm:p-0 overflow-y-auto">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl mx-auto max-h-[90vh] sm:max-h-none flex flex-col">
                {/* Modal Header */}
                <div className="flex justify-between items-center p-4 sm:p-6 border-b sticky top-0 bg-white rounded-t-lg z-10">
                ...
                {/* Error Message */}
                 {errorMessage && (
                    <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded m-4 sm:m-6" role="alert">
                    ...
                    <form id="item-form" onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
                        ...
                        <div className="border-b pb-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <FormInput name="sku" label="SKU" value={formData.sku} onChange={handleInputChange} required />
                                <FormInput name="huid" label="HUID" value={formData.huid} onChange={handleInputChange} />
                                ...
                                <div className="sm:col-span-2 space-y-2">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                        <img 
                                            className="h-24 w-24 sm:h-20 sm:w-20 rounded-lg object-cover bg-gray-100 border mx-auto sm:mx-0" 
                                        ...
                                        <div className="flex-1 w-full space-y-2">
                                            <label
                                                ...
                                                className="flex items-center justify-center gap-2 px-4 py-2 bg-white text-gray-700 border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-150 cursor-pointer w-full"
                                            >
                                                <IconCamera />
                        ...
                        <div className="border-b pb-6 space-y-4">
                            ...
                            {formData.metals.map((metal, index) => (
                                <div key={index} className="grid grid-cols-2 sm:grid-cols-6 gap-3 p-3 border rounded-lg bg-gray-50 relative">
                                    <FormSelect 
                                        name="type" 
                                        label="Type" 
                                        value={metal.type} 
                                        onChange={(e) => handleMetalChange(index, e)} 
                                        options={Object.keys(metalConfig)} 
                                    />
                                    <FormSelect 
                                        name="purity" 
                                        label="Purity" 
                                        value={metal.purity} 
                                        onChange={(e) => handleMetalChange(index, e)} 
                                        options={metalConfig[metal.type] || []} 
                                    />
                                    <FormInput name="netWeight" label="Net(g)" type="number" value={metal.netWeight} onChange={(e) => handleMetalChange(index, e)} />
                                    <FormInput name="grossWeight" label="Gross(g)" type="number" value={metal.grossWeight} onChange={(e) => handleMetalChange(index, e)} />
                                    <FormInput name="makingCharges" label="Making ($)" type="number" value={metal.makingCharges} onChange={(e) => handleMetalChange(index, e)} />
                        ...
                        <div className="space-y-4">
                            ...
                            {formData.stones.map((stone, index) => (
                                <div key={index} className="grid grid-cols-2 sm:grid-cols-6 gap-3 p-3 border rounded-lg bg-gray-50">
                                    <FormSelect name="type" label="Type" value={stone.type} onChange={(e) => handleStoneChange(index, e)} options={STONE_TYPES} />
                ...
                <div className="flex justify-end items-center gap-4 p-4 sm:p-6 border-t sticky bottom-0 bg-white rounded-b-lg z-10">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-4 sm:px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300"
                    >
                        Cancel
                    </button>
                    <button
                        ...
                        className="flex items-center justify-center gap-2 px-4 sm:px-6 py-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-blue-300 disabled:cursor-not-allowed"
                    >
                        {isUploading ? (
...
function FormSelect({ label, name, value, onChange, options, required = false, children }) {
    return (
        <div>
            ...
            >
                {/* Check if children props are provided (used for the dynamic options in POSView) */}
                {children} 
                
                {/* Fallback to simple array mapping if no children are provided */}
